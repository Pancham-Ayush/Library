...
EmailService emailService;
    ...
    public String resetpass(@RequestParam("phone") Long phone, Model model, HttpSession httpSession) {
        ...
        if (user.isPresent()) {
            emailService.sendEmail(user.get().getEmail(), "Forgot Password", "Reset Password\n OTP: " + otp);
            emailService.sendOtpSms("+91"+String.valueOf(phone),"Reset Password\n OTP: " + otp);
    ...
    public String verifyotp(@RequestParam("enteredotp") int enteredotp, HttpSession httpSession, Model model) {
        ...
        if (otp.equals(enteredotp)) {  // ✅ Use .equals() to compare Integer objects
            return "verifyotp";  // Proceed to password reset
        }
